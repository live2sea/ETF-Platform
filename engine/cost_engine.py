#!/usr/bin/env python3
#解释：该脚本定义了一个CostEngine类，用于计算ETF的持仓成本和盈亏情况。它从SQLite数据库中加载交易记录数据，计算每个ETF的持仓数量、平均成本、买入金额、卖出金额和已实现盈亏，并将计算结果保存到数据库中的持仓快照表中。最后，在主程序中创建一个CostEngine实例，执行计算并打印当前持仓情况。
